...
- Each row shows win/draw/win probabilities, favorite, match profile, and upset-or-draw probability. Completed matches from `src/liveResults.js` show final score, model hit/deviation, and points/goal-difference impact. Today's fixtures are also surfaced as a separate homepage rail before the full 72-match list. A second "明日比赛/Tomorrow" rail (`tomorrowMatches`/`tomorrowDate`, computed as the calendar day after `todayDate` via the `nextDate` helper, UTC, month/year-rollover safe) previews the next day's fixtures with the same card renderer, so the premium-lock and free-lean boundary are identical to the today rail.
...
**index.html GitHub sync — RESOLVED 2026-06-13**: the AdSense publisher ID is no longer hardcoded in index.html. `app.js` (`ensureAdsenseMeta` + `ensureAdSenseScript`) injects both the `google-adsense-account` meta and the adsbygoogle loader at runtime from `window.WorldCupAdConfig.adsenseClient` (set in server-only `config.local.js`). index.html now carries zero publisher ID and is committed to GitHub with full i18n. **Do not re-hardcode the pub ID or the loader `<script client=ca-pub-…>` back into index.html** — it would re-leak the ID into the repo. ads.txt (server-only) is the primary verification.
...
AdSense compliance route (2026-06-13): root domain `https://renrenrenai.cn/` now redirects to `https://renrenrenai.cn/worldcup/`, and `/worldcup/` serves the same static app from `/var/www/worldcup`. Keep `/ads.txt` available at the root domain because AdSense verifies `renrenrenai.cn/ads.txt`. Public copy should frame the app as independent pre-match research, not official, betting, or gambling content.
...
- 2026-06-13: AdSense compliance pass. Removed public FIFA-link/official-feeling references and betting/gambling trigger words from the app/README copy, changed the unlock CTA from an explicit ad-watching phrase to "查看高级分析", bumped JS cache versions, and deployed the root-domain `/worldcup/` route while preserving root `ads.txt`. Also fixed the update-log CSS selector that squeezed text into 8px columns and made match cards roomier. Verification: `npm test` 70 pass; live page at `https://renrenrenai.cn/worldcup/` has AdSense meta/script, no FIFA links, no banned terms, and normal update-log layout.
- 2026-06-13: Added verified finished results for the 2026-06-12 matchday from the ESPN scoreboard: B-1 Canada 1-1 Bosnia and Herzegovina, D-1 United States 4-1 Paraguay. `src/liveResults.js` now marks both fixtures final and carries redundant team keys for the result join. Bumped JS cache query strings for liveResults/matchAnalysis/realtimeData/app and verified `npm test` 70 pass before deploy.
